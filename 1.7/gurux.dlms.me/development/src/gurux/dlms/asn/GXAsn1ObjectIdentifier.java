...
public class GXAsn1ObjectIdentifier {
    ...
    static byte[] oidStringtoBytes(final String oid) {
        int value;
        List<String> arr = GXCommon.split(oid.trim(), '.');
        // Make first byte.
        GXByteBuffer tmp = new GXByteBuffer();
        value = Integer.parseInt(arr.get(0)) * 40;
        value += Integer.parseInt(arr.get(1));
        tmp.setUInt8(value);
        for (int pos = 2; pos != arr.size(); ++pos) {
            value = Integer.parseInt(arr.get(pos));
            if (value < 0x80) {
                tmp.setUInt8(value);
