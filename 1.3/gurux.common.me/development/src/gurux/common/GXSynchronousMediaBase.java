...
public class GXSynchronousMediaBase {
  ...
  /**
   * Position where end of packet was last time search. This is used to improve searching.
   */
  ...
  public static byte[] getAsByteArray(final Object value) {
    ...
    if (value instanceof Short) {
      return ByteBuffer.allocateDirect(GXCommon.SHORT_BYTES).putShort(((Short) value).shortValue())
          .array();
    }

    if (value instanceof Integer) {
      return ByteBuffer.allocateDirect(GXCommon.INTEGER_BYTES).putInt(((Integer) value).intValue())
          .array();
    }
    if (value instanceof String) {
      return java.nio.ByteBuffer.wrap(((String) value).getBytes()).array();
    }
   ...
   * @return Position where end of packet was found. -1 Is returned if data was not found in given
   *         time.
   */
  private int findData(final ReceiveParameters args, final boolean[] isFound) {
    boolean isReceived;
    int size = 0, foundPosition = -1;
    int lastBuffSize = 0;
    java.util.Calendar calendar = java.util.Calendar.getInstance();
    long startTime = calendar.getTime().getTime();
    byte[] terminator = null;
    int minSize = (int) Math.max(args.getCount(), size);
    ...
    isFound[0] = true;
    if (args.getEop() != null) {
      if (args.getEop() instanceof Object[]) {
        terminator = getAsByteArray(((Object[]) args.getEop())[0]);
      } else {
        terminator = getAsByteArray(args.getEop());
      }
      size = terminator.length;
    }
    ...
    do {
      ...
      synchronized (sync) {
        isReceived = !(lastBuffSize == receivedSize || receivedSize < minSize);
      }
    ...
    if (foundPosition != -1) {
      ...
      synchronized (sync) {
        ...
        if (foundPosition != 0) {
          ...
          data = new byte[foundPosition];
          System.arraycopy(receivedBuffer, 0, data, 0, foundPosition);
          // Remove read data.
      ...
      if (args.getReply() == null) {
        ...
        if (newArray != null) {
          ...
          int len = oldReplySize + newArray.length;
          byte[] arr = new byte[len];
          // Copy old values.
          System.arraycopy((byte[]) args.getReply(), 0, arr, 0, oldReplySize);
          // Copy new values.
          System.arraycopy(newArray, 0, arr, oldReplySize, newArray.length);
          args.setReply(arr);
        }
