...
class GXDLMSChipperingStream {
	...
	public void write(final byte[] input) {
		for (int pos = 0; pos != input.length; ++pos) {
			bufBlock[bytesRemaining++] = input[pos];
		...
		for (round = 0; round < 10; ++round) {
			...
			if (round < 9) {
				for (i = 0; i < 4; i++) {
					...
					data[buf4 + offset] = (byte) (data[buf4 + offset] ^ buf3
							^ buf1);
					buf3 = (byte) (data[buf4 + 1 + offset]
							^ data[buf4 + 2 + offset]);
					buf3 = galoisMultiply(buf3);
					data[buf4 + 1 + offset] = (byte) (data[buf4 + 1 + offset]
							^ buf3 ^ buf1);
					buf3 = (byte) (data[buf4 + 2 + offset]
							^ data[buf4 + 3 + offset]);
					buf3 = galoisMultiply(buf3);
					data[buf4 + 2 + offset] = (byte) (data[buf4 + 2 + offset]
							^ buf3 ^ buf1);
					buf3 = (byte) (data[buf4 + 3 + offset] ^ buf2);
					buf3 = galoisMultiply(buf3);
					data[buf4 + 3 + offset] = (byte) (data[buf4 + 3 + offset]
							^ buf3 ^ buf1);
				}
			}
