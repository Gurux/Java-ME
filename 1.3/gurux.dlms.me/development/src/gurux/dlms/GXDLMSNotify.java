...
public class GXDLMSNotify {
	...
	public final byte[][] generateDataNotificationMessages(final Date date,
		...
		while (iterator.hasMoreElements()) {
			...
			addData((GXDLMSObject) it.getKey(),
					((Integer) it.getValue()).intValue(), buff);
		}
	...
	public final byte[][] generatePushSetupMessages(final Date date,
		...
		Enumeration iterator = push.getPushObjectList().elements();
		while (iterator.hasMoreElements()) {
			GXSimpleEntry it = (GXSimpleEntry) iterator.nextElement();
			addData((GXDLMSObject) it.getKey(),
	 ...
	 */
	public final Vector parsePush(final Object[] data) {
		GXDLMSObject obj;
		int index;
		DataType dt;
		Object value;
		...
		if (data != null) {
			...
			Object[] arr = (Object[]) data[0];

			for (int pos = 0; pos != arr.length; ++pos) {
				Object[] tmp = (Object[]) arr[pos];
				int classID = GXCommon.intValue((tmp[0])) & 0xFFFF;
				if (classID > 0) {
					GXDLMSObject comp;
					comp = getObjects().findByLN(ObjectType.forValue(classID),
							GXDLMSObject.toLogicalName((byte[]) tmp[1]));
					if (comp == null) {
						comp = GXDLMSClient.createDLMSObject(classID,
								new Integer(0), 0, tmp[1], null);
						settings.getObjects().addElement(comp);
						c.updateOBISCodeInformation(comp);
					}
					if (comp.getClass() != GXDLMSObject.class) {
						items.addElement(new GXSimpleEntry(comp,
								new Integer(GXCommon.intValue(tmp[2]))));
					} else {
			...
			for (int pos = 0; pos < data.length; ++pos) {
				GXSimpleEntry k = (GXSimpleEntry) items.elementAt(pos);
				obj = (GXDLMSObject) k.getKey();
				value = data[pos];
				index = GXCommon.intValue(k.getValue());
				...
				ValueEventArgs e = new ValueEventArgs(settings, obj, index, 0,
						null);
				e.setValue(value);
				obj.setValue(settings, e);
				e.setValue(value);

				e = new ValueEventArgs(settings, (GXDLMSObject) k.getKey(),
						((Integer) k.getValue()).intValue(), 0, null);
				e.setValue(data[pos]);
				((GXDLMSObject) ((GXSimpleEntry) items.elementAt(pos)).getKey())
						.setValue(settings, e);
			}
		}
		return items;
	 ...
	 *            Received data.
	 */
	public final void parsePush(final Vector objects, final Object[] data) {
		GXDLMSObject obj;
		int index;
		DataType dt;
		Object value;
		...
		for (int pos = 0; pos < data.length; ++pos) {
			GXSimpleEntry k = (GXSimpleEntry) objects.elementAt(pos);
			obj = (GXDLMSObject) k.getKey();
			value = data[pos];
			index = ((Integer) k.getValue()).intValue();
			if (value instanceof byte[]) {
				dt = obj.getUIDataType(index);
				if (dt != DataType.NONE) {
					value = GXDLMSClient.changeType((byte[]) value, dt);
