...
final class GXDLMSSNCommandHandler {
	...
	private static void handleRead(final GXDLMSSettings settings,
			...
			final Vector list, final Vector reads, final Vector actions,
	...
	private static void handleReadBlockNumberAccess(
		...
		if (settings.getIndex() != settings.getCount() && server
			...
			for (int pos = 0; pos != server.getTransaction()
					.getTargets().length; ++pos) {
				ValueEventArgs it = server.getTransaction().getTargets()[pos];
				if (it.isAction()) {
			...
			if (reads.size() != 0) {
				ValueEventArgs[] tmp = new ValueEventArgs[reads.size()];
				reads.copyInto(tmp);
				server.notifyRead(tmp);
			}

			if (actions.size() != 0) {
				ValueEventArgs[] tmp = new ValueEventArgs[actions.size()];
				actions.copyInto(tmp);
				server.notifyAction(tmp);
			}
	...
	static void handleReadRequest(final GXDLMSSettings settings,
		...
		if (xml == null && data.size() == 0) {
			...
			for (int pos = 0; pos != server.getTransaction()
					.getTargets().length; ++pos) {
				list.addElement(server.getTransaction().getTargets()[pos]);
			}
		} else {
		...
		ValueEventArgs[] tmp = new ValueEventArgs[list.size()];
		list.copyInto(tmp);
		int requestType = getReadData(settings, tmp, bb);
		GXDLMSSNParameters p = new GXDLMSSNParameters(settings,
		...
		if (server.getTransaction() == null && (bb.size() != bb.position()
			...
			while (iterator.hasMoreElements()) {
				reads.addElement(iterator.nextElement());
			}
			ValueEventArgs[] tmp2 = new ValueEventArgs[reads.size()];
			reads.copyInto(tmp2);
			server.setTransaction(
					new GXDLMSLongTransaction(tmp2, Command.READ_REQUEST, bb));
	...
	private static GXSNInfo findSNObject(final GXDLMSServer server,
		...
		Enumeration iterator = server.getItems().elements();
		while (iterator.hasMoreElements()) {
	...
	static void handleWriteRequest(final GXDLMSSettings settings,
		...
		for (int pos = 0; pos != cnt; ++pos) {
			...
			if (xml != null) {
				...
				// If object has found.
				GXSNInfo target = (GXSNInfo) targets.elementAt(pos);
				value = GXCommon.getData(data, di);
				if (value instanceof byte[]) {
					DataType dt = target.getItem()
							.getDataType(target.getIndex());
