...
public class GXAsn1BitString {
     ...
     *            Amount of zero.
     */
    private static void appendZeros(final StringBuffer sb, final int count) {
    ...
    public GXAsn1BitString(final String bitString) {
        ...
        for (int pos = 0; pos != value.length; ++pos) {
        	char tmp[] = new char[8];
        	sb.getChars(8 * pos, 8 * (pos + 1),tmp, 8);
            value[pos] = (byte) Integer
                    .parseInt(new String(tmp), 2);
        }
    }
    ...
    public final String toString() {
        ...
        StringBuffer sb = new StringBuffer(8 * value.length);
        for (int pos = 0; pos != value.length; ++pos) {
            GXCommon.toBitString(sb, value[pos], 8);
        }
