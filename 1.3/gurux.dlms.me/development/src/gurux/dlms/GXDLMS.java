...
abstract class GXDLMS {
	...
	static GXDLMSObject createObject(final ObjectType type) {
		...
		case 21:// ObjectType.REGISTER_MONITOR:
			return new GXDLMSRegisterMonitor();
		case 61:// ObjectType.REGISTER_TABLE:
			return new GXDLMSObject();
		case 101:// ObjectType.ZIG_BEE_SAS_STARTUP:
			return new GXDLMSObject();
		case 102:// ObjectType.ZIG_BEE_SAS_JOIN:
			return new GXDLMSObject();
		case 17:// ObjectType.SAP_ASSIGNMENT:
			return new GXDLMSSapAssignment();
		case 10:// ObjectType.SCHEDULE:
			return new GXDLMSSchedule();
		case 9:// ObjectType.SCRIPT_TABLE:
			return new GXDLMSScriptTable();
		case 2:// ObjectType.SMTP_SETUP:
			return new GXDLMSObject();
		case 11:// ObjectType.SPECIAL_DAYS_TABLE:
			return new GXDLMSSpecialDaysTable();
		case 63:// ObjectType.STATUS_MAPPING:
			return new GXDLMSObject();
		case 41:// ObjectType.TCP_UDP_SETUP:
			return new GXDLMSTcpUdpSetup();
		case 103:// ObjectType.ZIG_BEE_SAS_APS_FRAGMENTATION:
			return new GXDLMSObject();
		case 26:// ObjectType.UTILITY_TABLES:
			return new GXDLMSObject();
		case 74:// ObjectType.MBUS_MASTER_PORT_SETUP:
			return new GXDLMSMBusMasterPortSetup();
		case 40:// ObjectType.PUSH_SETUP:
			return new GXDLMSPushSetup();
		case 60:// ObjectType.MESSAGE_HANDLER:
	...
	public static byte[] receiverReady(final GXDLMSSettings settings,
		...
		return (byte[]) reply.elementAt(0);
	}

	static String getDescription(final int errCode) {
		String str;
		switch (errCode) {
		case -2:// ErrorCode.REJECTED:
			str = "Rejected";
			break;
		case 0:// ErrorCode.OK:
			str = "";
	 ...
	 */
	public static Vector getLnMessages(final GXDLMSLNParameters p) {
		...
		do {
			...
			while (reply.position() != reply.size()) {
				if (p.getSettings()
					...
					messages.addElement(
							getWrapperFrame(p.getSettings(), reply));
				} else {
					messages.addElement(
	 ...
	 */
	public static Vector getSnMessages(final GXDLMSSNParameters p) {
		GXByteBuffer reply = new GXByteBuffer();
		Vector messages = new Vector();
		byte frame = 0x0;
		if (p.getCommand() == Command.AARQ) {
		...
		do {
			...
			while (reply.position() != reply.size()) {
				if (p.getSettings()
					...
					messages.addElement(
							getHdlcFrame(p.getSettings(), frame, reply));
					frame = 0;
				}
			}
			reply.clear();
	...
	static short getHdlcData(final boolean server,
		...
		if ((frame & HdlcFrameType.U_FRAME) == HdlcFrameType.U_FRAME) {
		...
		} else if ((frame & HdlcFrameType.S_FRAME) == HdlcFrameType.S_FRAME) {
			// If S-frame
			int tmp = (frame >> 2) & 0x3;
			// If frame is rejected.
			if (tmp == HdlcControlFrame.REJECT) {
				data.setError((short) ErrorCode.REJECTED.getValue());
			} else if (tmp == HdlcControlFrame.RECEIVE_NOT_READY) {
				data.setError((short) ErrorCode.REJECTED.getValue());
			} else if (tmp == HdlcControlFrame.RECEIVE_READY) {
		...
		if ((reply.getMoreData().getValue()
			...
			if (reply.getXml() != null) {
				...
				reply.getXml().appendLine(TranslatorTags.LAST_BLOCK, "Value",
						reply.getXml().integerToHex(new Boolean(lastBlock), 2));
	...
	static boolean handleReadResponse(final GXDLMSSettings settings,
		...
		for (int pos = 0; pos != cnt; ++pos) {
			...
			case SingleReadResponse.DATA:
				...
				if (reply.getXml() != null) {
					...
					values.addElement(reply.getValue());
					reply.setValue(null);
				}
				break;
		...
		if (values != null) {
			Object[] tmp = new Object[values.size()];
			values.copyInto(tmp);
			reply.setValue(tmp);
		}
		return cnt == 1;
	...
	private static void handleAccessResponse(final GXDLMSSettings settings,
		...
		if (reply.getXml() != null) {
			...
			// Get count
			int cnt = GXCommon.getObjectCount(reply.getData());
			Vector list = new Vector(cnt);
			for (int pos = 0; pos != cnt; ++pos) {
				getDataFromBlock(reply.getData(), start);
				getValueFromData(settings, reply);
				list.addElement(reply.getValue());
				reply.setValue(null);
			}
			Object[] tmp2 = new Object[list.size()];
			list.copyInto(tmp2);
			reply.setValue(tmp2);
		}
	}
	...
	private static void handleConfirmedServiceError(final GXReplyData data) {
		ConfirmedServiceError service = ConfirmedServiceError
				.forValue(data.getData().getUInt8());
