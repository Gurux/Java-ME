...
public class GXDLMSClient {
	...
	private static final Logger LOGGER = Logger
			.getLogger(GXDLMSClient.class.getName());
	...
	public final void parseUAResponse(final GXByteBuffer data) {
		...
		while (data.position() < data.size()) {
			...
			switch (len) {
			case 1:
				val = new Short(data.getUInt8());
				break;
			case 2:
				val = new Integer(data.getUInt16());
				break;
			case 4:
				val = new Long(data.getUInt32());
				break;
			default:
	...
	public final byte[][] aarqRequest() {
		...
		if (getAuthentication().getValue() > Authentication.LOW.getValue()) {
	...
	public final void parseAareResponse(final GXByteBuffer reply) {
		...
		isAuthenticationRequired = GXAPDU.parsePDU(settings,
				settings.getCipher(), reply,
	...
	public final byte[][] getApplicationAssociationRequest() {
		...
		return method(new Integer(0xFA00), ObjectType.ASSOCIATION_SHORT_NAME, 8,
				bb.array(), DataType.OCTET_STRING);
	}
		...
		if (value != null) {
			if (settings.getAuthentication() == Authentication.HIGH_ECDSA) {
				try {
					...
					//TODO: equals = ver.verify(GXAsn1Converter.encode(value));
	...
	static GXDLMSObject createDLMSObject(final int classID,
		...
		updateObjectData(obj, type, version, new Integer(baseName), (byte[]) ln,
				accessRights);
		return obj;
	}
	...
	private GXDLMSObjectCollection parseSNObjects(final GXByteBuffer buff,
		...
		for (long objPos = 0; objPos != cnt; ++objPos) {
			...
			int classID = GXCommon.intValue((objects[1])) & 0xFFFF;
			int baseName = GXCommon.intValue((objects[0])) & 0xFFFF;
			if (baseName > 0) {
	...
	static void updateObjectData(final GXDLMSObject obj,
		...
		if (accessRights instanceof Object[]
			...
			for (int pos = 0; pos != ((Object[]) access[0]).length; ++pos) {
				Object attributeAccess = ((Object[]) access[0])[pos];
			...
			for (int pos = 0; pos != ((Object[]) access[1]).length; ++pos) {
				Object methodAccess = ((Object[]) access[1])[pos];
				...
				if (((Object[]) methodAccess)[1] instanceof Boolean) {
					...
					// If version is 1.
					tmp = GXCommon.intValue(((Object[]) methodAccess)[1]);
				}
		...
		if (baseName != null) {
			obj.setShortName(GXCommon.intValue(baseName));
		}
		if (version != null) {
			obj.setVersion(GXCommon.intValue(version));
		}
	...
	public final GXDLMSObjectCollection parseObjects(final GXByteBuffer data,
		...
		GXCommon.addAll(settings.getObjects(), objects);
		return objects;
	}
	...
	private GXDLMSObjectCollection parseLNObjects(final GXByteBuffer buff,
		...
		for (long objPos = 0; objPos != cnt; ++objPos) {
			...
			int classID = GXCommon.intValue((objects[0])) & 0xFFFF;
			if (classID > 0) {
	 ...
	 *            Received values.
	 */
	public final void updateValues(final Vector list, final Vector values) {
		int pos = 0;
		...
		while (iterator.hasMoreElements()) {
			...
			ValueEventArgs e = new ValueEventArgs(settings,
					(GXDLMSObject) it.getKey(),
					((Integer) it.getValue()).intValue(), 0, null);
		    e.setValue(values.elementAt(pos));
			synchronized (it.getKey()) {
			    ((GXDLMSObject) it.getKey()).setValue(settings, e);
			}
			++pos;
		}
	}
	 ...
	 *            Received values.
	 */
	public final void updateValues(final Vector list, final Object[] values) {
		int pos = 0;
		Enumeration iterator = list.elements();
		while (iterator.hasMoreElements()) {
			GXSimpleEntry it = (GXSimpleEntry) iterator.nextElement();
	...
	public static Object changeType(final byte[] value, final DataType type) {
		...
		if (type == DataType.OCTET_STRING && ret instanceof byte[]) {
			...
			if (arr.length == 0) {
				...
				StringBuffer bcd = new StringBuffer(arr.length * 4);
	...
	public final byte[][] method(final Object name, final ObjectType objectType,
		...
		if (getUseLogicalNameReferencing()) {
			...
			// Add LN
			String[] items = GXCommon.split((String) name, '.');
			if (items.length != 6) {
				throw new IllegalArgumentException("Invalid Logical Name.");
			}
			for (int pos = 0; pos != items.length; ++pos) {
				attributeDescriptor
						.setUInt8(Integer.valueOf(items[pos]).byteValue());
			}
			// Attribute ID.
			...
			GXDLMSSNParameters p = new GXDLMSSNParameters(settings,
					Command.READ_REQUEST, 1, requestType, attributeDescriptor,
					data);
			reply = GXDLMS.getSnMessages(p);
		}
		return GXCommon.toArray(reply);
	 ...
	 */
	public final byte[][] writeList(final Vector list) {
		...
		if (this.getUseLogicalNameReferencing()) {
			...
			Enumeration iterator = list.elements();
			while (iterator.hasMoreElements()) {
				GXWriteItem it = (GXWriteItem) iterator.nextElement();
				// CI.
				bb.setUInt16(it.getTarget().getObjectType().getValue());
				String[] items = GXCommon.split(it.getTarget().getLogicalName(),
						'.');
				if (items.length != 6) {
					throw new IllegalArgumentException("Invalid Logical Name.");
				}
				for (int pos = 0; pos != items.length; ++pos) {
					bb.setUInt8(Integer.valueOf(items[pos]).byteValue());
				}
				// Attribute ID.
			...
			while (iterator.hasMoreElements()) {
				...
				// Add variable type.
				bb.setUInt8(2);
				int sn = it.getTarget().getShortName();
				sn += (it.getIndex() - 1) * 8;
				bb.setUInt16(sn);
			}
		}
	 ...
	 * @return Read request as byte array.
	 */
	public final byte[][] readList(final Vector list) {
		if (list == null || list.isEmpty()) {
			throw new IllegalArgumentException("Invalid parameter.");
		}
		int pos;
		...
		if (this.getUseLogicalNameReferencing()) {
			...
			int count = (settings.getMaxPduSize() - 12) / 10;
			pos = 0;
			if (list.size() < count) {
				count = list.size();
			}
			...
			while (iterator.hasMoreElements()) {
				...
				// CI.
				data.setUInt16(((GXDLMSObject) it.getKey()).getObjectType()
						.getValue());
				String[] items = GXCommon.split(
						((GXDLMSObject) it.getKey()).getLogicalName(), '.');
				if (items.length != 6) {
					throw new IllegalArgumentException("Invalid Logical Name.");
				}
				for (int pos1 = 0; pos1 != items.length; ++pos1) {
					data.setUInt8(Integer.valueOf(items[pos1]).byteValue());
				}
				// Attribute ID.
				data.setUInt8(((Integer) it.getValue()).intValue());
				// Attribute selector is not used.
				data.setUInt8(0);

				++pos;
				if (pos % count == 0 && list.size() != pos) {
					GXCommon.addAll(messages, GXDLMS.getLnMessages(p));
					data.clear();
			...
			while (iterator.hasMoreElements()) {
				...
				int sn = ((GXDLMSObject) it.getKey()).getShortName();
				sn += (((Integer) it.getValue()).intValue() - 1) * 8;
				data.setUInt16(sn);
			}
			GXCommon.addAll(messages, GXDLMS.getSnMessages(p));
		}
		return GXCommon.toArray(messages);
	}
	 ...
	 */
	public final byte[][] readRowsByEntry(final GXDLMSProfileGeneric pg,
			final int index, final int count, final Vector columns) {
		...
		buff.setUInt8(0x04);
		// Add start index
		GXCommon.setData(buff, DataType.UINT32, new Integer(index));
		// Add Count
		GXCommon.setData(buff, DataType.UINT32, new Integer(count));
		...
		if (columns != null && columns.size() != 0) {
			...
			columnCount = 1;
			Enumeration iterator = columns.elements();
			while (iterator.hasMoreElements()) {
				GXSimpleEntry c = (GXSimpleEntry) iterator.nextElement();
				pos = 0;
				boolean found = false;
				Enumeration iterator2 = pg.getCaptureObjects().elements();
				while (iterator2.hasMoreElements()) {
					GXSimpleEntry it = (GXSimpleEntry) iterator2.nextElement();
					++pos;
					if (((GXDLMSObject) it.getKey())
							.getObjectType() == ((GXDLMSObject) c.getKey())
									.getObjectType()
							&& ((GXDLMSObject) it.getKey()).getLogicalName()
									.compareTo(((GXDLMSObject) c.getKey())
											.getLogicalName()) == 0
							&& ((GXDLMSCaptureObject) it.getValue())
									.getAttributeIndex() == ((GXDLMSCaptureObject) c
											.getValue()).getAttributeIndex()
							&& ((GXDLMSCaptureObject) it.getValue())
									.getDataIndex() == ((GXDLMSCaptureObject) c
											.getValue()).getDataIndex()) {
						found = true;
						if (pos < columnIndex) {
							columnIndex = pos;
						}
		...
		GXCommon.setData(buff, DataType.UINT16, new Integer(columnIndex));
		GXCommon.setData(buff, DataType.UINT16, new Integer(columnCount));
	 ...
	 */
	public final byte[][] readRowsByRange(final GXDLMSProfileGeneric pg,
			final Calendar start, final Calendar end, final Vector columns) {
		return readByRange(pg, start, end, columns);
	}
	...
	private byte[][] readByRange(final GXDLMSProfileGeneric pg,
			final Object start, final Object end, final Vector columns) {
		settings.resetBlockIndex();
		GXDateTime s = GXCommon.getDateTime(start);
		GXDateTime e = GXCommon.getDateTime(end);
		s.getSkip().addElement(DateTimeSkips.DAY_OF_WEEK);
		e.getSkip().addElement(DateTimeSkips.DAY_OF_WEEK);
		GXDLMSObject sort = pg.getSortObject();
		if (sort == null && pg.getCaptureObjects().size() != 0) {
			sort = (GXDLMSObject) ((GXSimpleEntry) pg.getCaptureObjects()
					.elementAt(0)).getKey();
		}
		...
		GXCommon.setData(buff, DataType.UINT16,
				new Integer(sort.getObjectType().getValue()));
		// LN
		GXCommon.setData(buff, DataType.OCTET_STRING, sort.getLogicalName());
		// Add attribute index.
		GXCommon.setData(buff, DataType.INT8, new Integer(2));
		// Add version
		GXCommon.setData(buff, DataType.UINT16, new Integer(sort.getVersion()));
		...
		if (columns == null) {
			...
			while (iterator.hasMoreElements()) {
				...
				// CI
				GXCommon.setData(buff, DataType.UINT16,
						new Integer(((GXDLMSObject) it.getKey()).getObjectType()
								.getValue()));
				// LN
				...
				GXCommon.setData(buff, DataType.INT8,
						new Integer(((GXDLMSCaptureObject) it.getValue())
								.getAttributeIndex()));
				// Add data index.
				GXCommon.setData(buff, DataType.INT16, new Integer(
