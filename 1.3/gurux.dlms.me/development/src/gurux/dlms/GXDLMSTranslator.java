...
public class GXDLMSTranslator {
	...
	private Hashtable tagsByName = new Hashtable();
	...
	static void addTag(final Hashtable list, final int value,
			final String text) {
		list.put(new Integer(value), text);
	}
	...
	private static void getTags(final TranslatorOutputType type,
			final Hashtable list, final Hashtable tagsByName) {
	...
	private static void handleAarqAare(final Node node,
		...
		case 0x89:
			...
			if (s.getOutputType() == TranslatorOutputType.SIMPLE_XML) {
				s.getSettings().setAuthentication(
						Authentication.valueOf(getValue(node, s)));
			} else {
		...
		case 0xBE05:
			...
			if (s.getOutputType() == TranslatorOutputType.STANDARD_XML) {
				...
				String[] tmp2 = GXCommon.split(nodes, " ");
				for (int pos = 0; pos != tmp2.length; ++pos) {
					String it = tmp2[pos];
					if (it.trim().compareTo("") != 0) {
	...
	private static void readNode(final Node node, final GXDLMSXmlSettings s) {
		...
		int tag = ((Integer) s.getTags().get(str)).intValue();
		if (s.getCommand() == Command.NONE) {
			...
			case Command.READ_RESPONSE << 8
				...
				ErrorCode err = valueOfErrorCode(s.getOutputType(),
						getValue(node, s));
				s.setCount(s.getCount() + 1);
	...
	private static GXByteBuffer updateDataType(final Node node,
		...
		case 13:// DataType.BCD:
			GXCommon.setData(s.getData(), DataType.BCD,
					new Short(s.parseShort(getValue(node, s))));
			break;
		...
		case 5:// DataType.INT32:
			GXCommon.setData(s.getData(), DataType.INT32,
					new Integer(s.parseInt(getValue(node, s))));
			break;
		case 20:// DataType.INT64:
			GXCommon.setData(s.getData(), DataType.INT64,
					new Long(s.parseLong(getValue(node, s))));
			break;
		...
		case 0x12:// DataType.UINT16:
			GXCommon.setData(s.getData(), DataType.UINT16,
					new Short(Short.parseShort(getValue(node, s), 16)));
			break;
	...
	private static void getFloat32(final Node node, final GXDLMSXmlSettings s) {
		GXByteBuffer bb = new GXByteBuffer();
		bb.setHexString(getValue(node, s));
		GXCommon.setData(s.getData(), DataType.FLOAT32,
	...
	private static void getFloat64(final Node node, final GXDLMSXmlSettings s) {
		GXByteBuffer bb = new GXByteBuffer();
		bb.setHexString(getValue(node, s));
		GXCommon.setData(s.getData(), DataType.FLOAT64,
				new Double(bb.getDouble()));
	}
