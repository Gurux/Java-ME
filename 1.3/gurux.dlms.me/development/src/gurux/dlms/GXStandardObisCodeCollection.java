 ...
 */
class GXStandardObisCodeCollection extends Vector {

	/*
	 * Convert Logical name string to bytes.
	 */
	static int[] getBytes(final String ln) {
		String[] tmp = GXCommon.split(ln, '.');
		if (tmp.length != 6) {
	...
	private boolean equalsInterface(final GXStandardObisCode it, final int ic) {
		...
		String str = String.valueOf(ic);
		String[] tmp = GXCommon.split(it.getInterfaces(), ',');
		for (int pos = 0; pos != tmp.length; ++pos) {
			if (tmp[pos].equalsIgnoreCase(str)) {
				return true;
			}
		}
		return false;
	...
	private static boolean equalsMask(final String obis, final int ic) {
		boolean number = true;
		if (obis.indexOf(',') != -1) {
			String[] tmp = GXCommon.split(obis, ',');
			for (int pos = 0; pos != tmp.length; ++pos) {
				String it = tmp[pos];
				if (it.indexOf('-') != -1) {
					if (equalsMask(it, ic)) {
	...
	public static boolean equalsMask(final String obisMask, final String ln) {
		return equalsObisCode(GXCommon.split(obisMask, '.'), getBytes(ln));
	}
	...
	public final GXStandardObisCode[] find(final int[] obisCode, final int ic) {
		...
		while (iterator.hasMoreElements()) {
			GXStandardObisCode it = (GXStandardObisCode) iterator.nextElement();
			...
			if (equalsInterface(it, ic)
				...
				list.addElement(tmp);
				String[] tmp2 = GXCommon.split(it.getDescription(), ';');
				if (tmp2.length > 1) {
					String desc = "";
                    if ("$1".equals(tmp2[1].trim())) {
					...
					if (!desc.equals("")) {
						Vector tmp3 = new Vector(tmp2.length);
						GXCommon.addAll(tmp3, tmp2);
						tmp3.setElementAt(desc, 1);
						StringBuffer builder = new StringBuffer();
						Enumeration iterator2 = tmp3.elements();
						while (iterator2.hasMoreElements()) {
							String s = (String) iterator2.nextElement();
        ...
        GXStandardObisCode[] tmp2 = new GXStandardObisCode[list.size()];
