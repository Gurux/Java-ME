...
final class GXDLMSLNCommandHandler {
				...
				if (moreData) {
					...
					if (bb.size() < settings.getMaxPduSize()) {
						...
						for (int pos = 0; pos != server.getTransaction()
							...
							ValueEventArgs arg = server.getTransaction()
									.getTargets()[pos];
	...
	private static void getRequestWithList(final GXDLMSSettings settings,
		...
		ValueEventArgs[] tmp2 = new ValueEventArgs[list.size()];
		list.copyInto(tmp2);
		server.notifyRead(tmp2);
		Object value;
		pos = 0;
		Enumeration iterator = list.elements();
		while (iterator.hasMoreElements()) {
			ValueEventArgs it = (ValueEventArgs) iterator.nextElement();
			try {
				if (it.getHandled()) {
					value = it.getValue();
				} else {
			...
			if (settings.getIndex() != settings.getCount()) {
				...
				list.copyInto(tmp);
				server.setTransaction(new GXDLMSLongTransaction(tmp,
		...
		if (obj == null) {
			...
			if (am != AccessMode.WRITE && am != AccessMode.READ_WRITE) {
				...
				try {
					...
					ValueEventArgs e = new ValueEventArgs(settings, obj, index,
							0, null);
					e.setValue(value);
	...
	static void handleMethodRequest(final GXDLMSSettings settings,
		...
		if (obj == null) {
			...
			if (obj.getMethodAccess(id) == MethodAccessMode.NO_ACCESS) {
				...
				ValueEventArgs e = new ValueEventArgs(settings, obj, id, 0,
						parameters);
