...
public class GXDLMSClock extends GXDLMSObject implements IGXDLMSBase {
	...
	public GXDLMSClock() {
		...
		value.addElement(DateTimeSkips.MONTH);
		value.addElement(DateTimeSkips.DAY);
		begin.setSkip(value);
	 ...
	 */
	public final void setStatus(final Vector value) {
		status = value;
	}
	...
	public final byte[] invoke(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 1) {
			...
			if (s > 30) {
				tm.setTime(
						new Date(tm.getTime().getTime() + GXDateTime.MINUTE));
			}
			tm.set(java.util.Calendar.SECOND, 0);
			tm.set(java.util.Calendar.MILLISECOND, 0);
			dt.setValue(tm.getTime());
			...
			// Shifts the time.
			int shift = GXCommon.intValue(e.getParameters());
			GXDateTime dt = getTime();
			java.util.Calendar tm = Calendar.getInstance();
			tm.setTime(dt.getValue());
			tm.setTime(new Date(
					tm.getTime().getTime() + (shift * GXDateTime.SECOND)));
			dt.setValue(tm.getTime());
			setTime(dt);
		} else {
	...
	public final Object getValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 9) {
			return new Integer(getClockBase().getValue());
		}
		e.setError(ErrorCode.READ_WRITE_DENIED);
		return null;
	}
	...
	public final void setValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 1) {
			...
			if (e.getValue() == null) {
				...
				setTimeZone(GXCommon.intValue(e.getValue()));
			}
		} else if (e.getIndex() == 4) {
			if (e.getValue() == null) {
				Vector val = new Vector();
				val.addElement(ClockStatus.OK);
				setStatus(val);
			} else {
				setStatus(
						ClockStatus.forValue(GXCommon.intValue(e.getValue())));
			}
		} else if (e.getIndex() == 5) {
			if (e.getValue() == null) {
			...
			if (e.getValue() == null) {
				...
				setDeviation(GXCommon.intValue(e.getValue()));
			}
			...
			if (e.getValue() == null) {
				...
				setClockBase(
						ClockBase.forValue(GXCommon.intValue(e.getValue())));
			}
		} else {
			e.setError(ErrorCode.READ_WRITE_DENIED);
		}
