...
public class GXDLMSObject {
	private Hashtable readTimes = new Hashtable();
	private int version;
	...
	protected GXDLMSObject(final ObjectType type, final String ln,
		...
		if (ln != null) {
			String[] items = GXCommon.split(ln, '.');
			if (items.length != 6) {
	...
	protected final java.util.Date getLastReadTime(int attributeIndex) {
		if (readTimes.containsKey(new Integer(attributeIndex))) {
			return (java.util.Date) readTimes.get(new Integer(attributeIndex));
		}
	...
	protected final void setLastReadTime(final int attributeIndex,
		...
		readTimes.put(new Integer(attributeIndex), tm);
	}
	...
	public final Object getName() {
		if (getShortName() != 0) {
			return new Integer(getShortName());
		}
	...
	public int getMethodCount() {
		throw new RuntimeException("getMethodCount");
	}
	// CHECKSTYLE:ON
	...
	public final void setMethodAccess(final int index,
		...
		if (att == null) {
			att = new GXDLMSAttributeSettings(index);
			getMethodAttributes().addElement(att);
		}
	...
	public int getAttributeCount() {
		throw new RuntimeException("getAttributeCount");
	}
	// CHECKSTYLE:ON
	...
	public void setValue(final GXDLMSSettings settings, ValueEventArgs e) {
		if (e.getIndex() == 1) {
			...
			throw new RuntimeException("setValue");
		}
	}
	// CHECKSTYLE:ON
