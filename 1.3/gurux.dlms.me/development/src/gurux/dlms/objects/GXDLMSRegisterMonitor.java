	...
	public final Object getValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 4) {
			...
			if (actions == null) {
				...
				for (int pos = 0; pos != actions.length; ++pos) {
					GXDLMSActionSet it = actions[pos];
	...
	public final void setValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 1) {
			...
			getMonitoredValue().setObjectType(ObjectType
					.forValue(GXCommon.intValue(((Object[]) e.getValue())[0])));
			getMonitoredValue().setLogicalName(GXDLMSClient
					.changeType((byte[]) ((Object[]) e.getValue())[1],
							DataType.OCTET_STRING)
					.toString());
			getMonitoredValue().setAttributeIndex(
					GXCommon.intValue(((Object[]) e.getValue())[2]));
		} else if (e.getIndex() == 4) {
			setActions(new GXDLMSActionSet[0]);
			if (e.getValue() != null) {

				Object[] arr = (Object[]) e.getValue();
				actions = new GXDLMSActionSet[arr.length];
					...
					set.getActionUp()
							.setLogicalName(
									GXDLMSClient
											...
											.toString());
					set.getActionUp()
							.setScriptSelector(GXCommon.intValue(target[1]));
