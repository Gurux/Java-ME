...
public class GXDLMSPushSetup extends GXDLMSObject implements IGXDLMSBase {
	private Vector pushObjectList;
	private GXSendDestinationAndMethod sendDestinationAndMethod;
	private Vector communicationWindow;
	...
	public GXDLMSPushSetup(final String ln, final int sn) {
		...
		pushObjectList = new Vector();
		sendDestinationAndMethod = new GXSendDestinationAndMethod();
		communicationWindow = new Vector();
	}
	 ...
	 */
	public final Vector getPushObjectList() {
	 ...
	 */
	public final Vector getCommunicationWindow() {
	...
	public final Object getValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 2) {
			...
			// for (Entry<GXDLMSObject, GXDLMSCaptureObject> it :
			// pushObjectList) {
			Enumeration iterator = pushObjectList.elements();
			while (iterator.hasMoreElements()) {
				...
				GXCommon.setData(buff, DataType.INT8,
						new Integer(((GXDLMSCaptureObject) it.getValue())
								.getAttributeIndex()));
				GXCommon.setData(buff, DataType.UINT16, new Integer(
		...
		if (e.getIndex() == 4) {
			...
			Enumeration iterator = communicationWindow.elements();
			while (iterator.hasMoreElements()) {
				GXSimpleEntry it = (GXSimpleEntry) iterator.nextElement();
				buff.setUInt8(DataType.STRUCTURE.getValue());
				buff.setUInt8(2);
	...
	public final void setValue(final GXDLMSSettings settings,
		...
		if (e.getIndex() == 1) {
			...
			pushObjectList.removeAllElements();
			if (e.getValue() instanceof Object[]) {
				...
				for (int pos = 0; pos != arr.length; ++pos) {
					...
					co.setAttributeIndex(GXCommon.intValue(tmp[2]));
					co.setDataIndex(GXCommon.intValue(tmp[3]));

					pushObjectList.addElement(new GXSimpleEntry(obj, co));
				}
			}
			...
			if (tmp != null) {
				sendDestinationAndMethod.setService(
						ServiceType.forValue(GXCommon.intValue(tmp[0])));
				sendDestinationAndMethod
						.setDestination(new String((byte[]) tmp[1]));
				sendDestinationAndMethod.setMessage(
						MessageType.forValue(GXCommon.intValue(tmp[2])));
			}
		} else if (e.getIndex() == 4) {
			communicationWindow.removeAllElements();
			if (e.getValue() instanceof Object[]) {
				Object[] arr = (Object[]) e.getValue();
				for (int pos = 0; pos != arr.length; ++pos) {
			...
			randomisationStartInterval = GXCommon.intValue(e.getValue());
		} else if (e.getIndex() == 6) {
			numberOfRetries = GXCommon.intValue(e.getValue());
		} else if (e.getIndex() == 7) {
			repetitionDelay = GXCommon.intValue(e.getValue());
		} else {
			e.setError(ErrorCode.READ_WRITE_DENIED);
		}
	}
