...
class GXReceiveThread extends Thread {
     ...
     *            Input stream.
     */
    GXReceiveThread(final GXSerial parent, final InputStream input) {
        super("GXSerial " + parent.getName());
        is = input;
        parentMedia = parent;
    }
    ...
    private void handleReceivedData(final byte[] buffer) {
        ...
        if (parentMedia.isSynchronous()) {
            ...
            synchronized (parentMedia.getSyncBase().getSync()) {
                parentMedia.getSyncBase().appendData(buffer, 0,
                        len);
                // Search End of Packet if given.
                if (parentMedia.getEop() != null) {
                    if (parentMedia.getEop() instanceof Object[]) {
                        Object[] arr = (Object[]) parentMedia.getEop();
                        for (int pos = 0; pos != arr.length; ++pos) {
                            totalCount = GXSynchronousMediaBase.indexOf(buffer,
                                    GXSynchronousMediaBase.getAsByteArray(arr[pos]),
                                    0, len);
                            if (totalCount != -1) {
                                break;
                            }
